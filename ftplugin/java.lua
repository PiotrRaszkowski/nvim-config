...
local function get_java_home()
	...
	if vim.fn.isdirectory(java_home) == 0 then
		vim.notify(
			"Java home not found at: " .. java_home .. "\nPlease update the path in ftplugin/java.lua",
...
local function get_jdtls()
	...
	local jdtls_path = vim.fn.stdpath("data") .. "/mason/packages/" .. jdtls_package.name
...
local function get_bundles()
	...
	-- Java Debug Adapter - using $MASON/share directory (stable interface)
	local java_debug_jars = vim.fn.globpath(
		vim.fn.expand("$MASON/share/java-debug-adapter"),
		"*.jar",
		true,
		true
	)
	...
	-- Java Test - using $MASON/packages (if share/ not available)
	local java_test_jars = vim.fn.globpath(
...
local cmd = {
	...
	"-Dlog.protocol=true",
	"-Dlog.level=WARNING", -- Reduced logging for better performance
	"-Xmx2G", -- Memory allocation (more is not always better - large heap = longer GC pauses)
	"-Xms1G",
    "-XX:+UseG1GC",
...
local settings = {
	java = {
		...
		eclipse = {
			downloadSources = false, -- Disabled for better performance (download manually if needed)
		},
		maven = {
			downloadSources = false, -- Disabled for better performance (download manually if needed)
		},
		...
		configuration = {
			updateBuildConfiguration = "interactive", -- Changed from "automatic" for better performance
		},
		inlayHints = {
			parameterNames = {
...
local config = {
	...
	on_attach = function(client, bufnr)
		...
		-- Code lens refresh disabled for performance
		-- Manual refresh available via :lua vim.lsp.codelens.refresh()
		-- Uncomment below to enable auto-refresh (may impact performance):
