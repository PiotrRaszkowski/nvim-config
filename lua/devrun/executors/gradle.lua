...
function M.stop(task_info)
	...
	if task_info.process then
		...
		if task_info.pid then
			vim.notify(string.format("Stopping Gradle process group (PID: %d)...", task_info.pid), vim.log.levels.INFO)
			...
			if not is_process_alive(task_info.pid) then
				vim.notify("Gradle process terminated gracefully", vim.log.levels.INFO)
				return true
			end
		end
